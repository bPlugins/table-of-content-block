...
const SelectTokenField = ({ multiple = true, defaultValue, value, onChange = () => {}, options = [] }) => {
  ...
  return (
    <div className="bPl-token-field-main-wrapper">
      <div className="bPl-single-token-field-wrapper" {...(multiple ? { style: { width: "100%" } } : {})} ref={toggleRef}>
        {!multiple && (
          <div onClick={() => setToggle(!toggle)} className={`bPl-single-token-field ${toggle ? "activeField" : ""}`}>
            {activeItem ? <span className="bPl-token-field-value">{activeItem.label}</span> : <span className="bPl-token-field-value">Select an option</span>}
            ...
            <span onClick={() => setToggle(!toggle)} style={{ transform: toggle ? "rotate(180deg)" : "rotate(0deg)" }} className="bPl-arrow-icon">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" height="14px">
        ...
        {multiple && (
          <div className="bPl-token-multiple-filed-wrapper">
            <label htmlFor={`searchText${id}`}>
              <div onClick={() => setToggle(true)} className={`bPl-token-multiple-filed-wrapper-item ${toggle ? "focusItem" : ""}`} style={{ cursor: "text" }}>
                ...
                {activeMultipleItem?.map((item, i) => (
                  <div key={i} className={`bPl-multiple-token-field`}>
                    <span className="bPl-multiple-token-field-value">{item.label}</span>
